...
X = mat73.loadmat('./Data/pre_zscore.mat')
theX = X['Vol_z']
...
#    scio.savemat('./Python/kfold/all/shuffled_index_cv'+str(cv), {'train_idx': train_idx,'test_idx': test_idx })
        ...
        for epoch in range(EPOCH):
            for step, (b_x, b_y) in enumerate(train_loader):  # gives batch data
                ...
                if epoch >=40 and tr_accuracy>0.8:
                    ...
                    train_los.append(float(loss))

                    if F1 >= auc_baseline and sen_1 >= 0.00 and sen_2 >= 0.00 and sen_3 >= 0.00:
                        auc_baseline = F1
                        torch.save(model.state_dict(),'./Python/model_z/model_all_cv'+str(cv)+'_'+str(start)+'_'+ str(end)+'.pth')
                        ...
                        qualified.append([accuracy, sen_1, sen_2, sen_3, pre_1, pre_2, pre_3, F1])
...
for cv in [1,2,3,4,5]:
    ...
    model.load_state_dict(torch.load(
        './Python/model_z/model_all_cv'+str(cv)+'_'+str(start)+'_'+ str(end)+'.pth'))
    model.cuda()
    model.eval()
    with torch.no_grad():
        for i, (test_x, test_y) in enumerate(test_loader):
...
#scio.savemat('./results for matlab/'+str(neg)+'vs'+str(pos), {'pre_score_all': pre_score_all, 'test_y_all': test_y_all})
...
for i in range(1000):
    ...
    shuffle(permu_index)
    shuffle(permu_index)
    correct = (np.array(predicted_all) == np.array(permu_index)).sum()
    accuracy = float(correct) / float(len(test_y_all))
    sen_1 = metrics.recall_score(permu_index, predicted_all, labels=[0], average='macro')
    sen_2 = metrics.recall_score(permu_index, predicted_all, labels=[1], average='macro')
    sen_3 = metrics.recall_score(permu_index, predicted_all, labels=[2], average='macro')
    pre_1 = metrics.precision_score(permu_index, predicted_all, labels=[0], average='macro')
    pre_2 = metrics.precision_score(permu_index, predicted_all, labels=[1], average='macro')
    pre_3 = metrics.precision_score(permu_index, predicted_all, labels=[2], average='macro')
    F1 = metrics.f1_score(permu_index, predicted_all, average='macro')
    permu_results.append([accuracy, sen_1, sen_2, sen_3, pre_1, pre_2, pre_3, F1])
    #correct = (np.array(predicted_all) == np.array(permu_index)).sum()
    #correct = (np.array(predicted_all) == np.array(permu_index)).sum()
    #accuracy = float(correct) / float(len(permu_index))
    #sen_1 = metrics.recall_score(permu_index, predicted_all, labels=[0], average='macro')
    #sen_2 = metrics.recall_score(permu_index, predicted_all, labels=[1], average='macro')
    #sen_3 = metrics.recall_score(permu_index, predicted_all, labels=[2], average='macro')
    #pre_1 = metrics.precision_score(permu_index, predicted_all, labels=[0], average='macro')
...
for metric in range(8):
    ...
    for r in range(1000):
        if permu_results[r][metric]>=qual_all[0][metric]:
            count=count+1
    p_values[metric]=count/1000
print(p_values)

temp = scio.loadmat('./permutation/permu_all.mat')
permu_results_all = temp['permu_results_all']
p_values=np.zeros([8])
for metric in range(8):
    count = 0
    for r in range(1000):
        if permu_results_all[r][metric]>=qual_all[0][metric]:
            count=count+1
    p_values[metric]=count/1000
print(p_values)