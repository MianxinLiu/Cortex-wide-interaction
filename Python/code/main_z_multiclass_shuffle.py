...
for start in [-250, -225, -200, -175, -150, -125, -100, -75, -50]:
    permu_results = []

    X = mat73.loadmat('/Data/pre_zscore.mat')
    # X = scio.loadmat('/media/user/4TB/zscore_datamatrix.mat')
    X = X['Vol_z']
    ...
    for r in range(100):
                ...
                for step, (b_x, b_y) in enumerate(train_loader):  # gives batch data
                    ...
                    if epoch >= 40 and tr_accuracy > 0.8:
                        ...
                        if accuracy >= auc_baseline and sen_1 >= 0.0 and sen_2 >= 0.0 and sen_3 >= 0.0:
                            ...
                            torch.save(model.state_dict(), './Python/model_z/temp25_cv' + str(cv) + '.pth')
        ...
        for cv in [1, 2, 3, 4, 5]:
            ...
            model.load_state_dict(torch.load(
                './Python/model_z/temp25_cv' + str(cv) + '.pth'))
            model.cuda()
            model.eval()
            with torch.no_grad():
                for i, (test_x, test_y) in enumerate(test_loader):
    ...
    +'_'+str(start)+'_'+ str(end) +'.mat', {'permu_results': permu_results, 'p_values': p_values})